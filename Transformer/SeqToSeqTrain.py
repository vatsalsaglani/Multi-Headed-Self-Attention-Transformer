...
from .SeqToSeq import SequnceToSequence
...
class FitSequenceToSequence(object):
    ...
    def prepare_model(self, pretrained = None):
        ...
        if pretrained:
            model = SequnceToSequence(emb = self.embedding_size, heads=self.heads, depth=self.depth, seq_length=self.seq_length, num_tokens=self.num_tokens, num_op_tokens=self.num_op_tokens, device=self.device, mask = self.mask, wide = self.wide)
            model = torch.load(pretrained, map_location=self.device)
            return model 
        else:
            model = SequnceToSequence(emb = self.embedding_size, heads=self.heads, depth=self.depth, seq_length=self.seq_length, num_tokens=self.num_tokens, num_op_tokens=self.num_op_tokens, device=self.device, mask = self.mask, wide = self.wide).to(device=self.device)
            return model 
